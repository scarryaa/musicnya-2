...
import { createModalArtistStore } from '../../../stores/api-store'
...
export const Artist = () => {
  ...
  const modalArtistStore = createModalArtistStore()
  const artistData = modalArtistStore({ id: store.app.modal.id })
  ...
  createEffect(async () => {
    ...
    if (store.app.modal.type.includes('library-')) {
      ...
      setArtistId(res.data[0].id)
    } else {
      setArtistId(store.app.modal.id)
    }
        ...
        <div class={styles.artist}>
          <div class={styles.artist__header}>
            <MediaItemDetailed
              ...
              title={currentArtist()?.attributes?.name}
              artists={currentArtist().attributes?.curatorName}
            />
          </div>
          <TabSwitcher
            tabs={[
              {
                name: 'Details',
                content: (
                  <div class={styles.artist__details}>
                    ...
                    >
                      <For each={currentArtist().attributes?.genreNames}>
                        {genre => (
                          <Chip
                            text={genre}
                    ...
                    <h3 class={styles.artist__details__header}>Info</h3>
                    <div class={styles.artist__details__grouping}>
                      <h4 class={styles.artist__details__subsubheader}>Id</h4>
                      <span class={styles.artist__details__description}>
                        {currentArtist().id}
                      </span>
                    </div>
                  </div>
              ...
              },
              {
                name: 'Albums',
                content: (
                  <div class={styles.artist__albums}>
                    <For each={currentArtist().relationships.albums.data}>
                      {album => (
                        <A
                          activeClass=""
                          onClick={() =>
                          ...
                          class={styles.artist__albums__album}
                  ...
                  <div class={styles.artist__artwork}>
                    <For each={artworkUrls()}>
                      {artwork =>
                        ...
                        artwork?.url && (
                          <div class={styles.artist__artwork__container}>
                            <h3 class={styles.artist__artwork__header}>{artwork.name}</h3>
                            <A href={artwork.url} target="_blank">
                              <img src={artwork.thumbnailUrl} />
                            </A>
                            <div class={styles.artist__artwork__colors}>
                              {artwork.bgColor && (
