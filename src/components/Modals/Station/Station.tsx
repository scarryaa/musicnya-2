...
import { createModalStationStore } from '../../../stores/api-store'
...
export const Station = () => {
  const [stationId, setStationId] = createSignal(store.app.modal.id)
  const modalStationStore = createModalStationStore()
  const stationData = modalStationStore({ id: store.app.modal.id })
  ...
  createEffect(() => {
    ...
    setStationId(store.app.modal.id)
    ...
    >
      <Match when={stationData.state === 'ready' && currentStation()}>
        <div class={styles.station}>
          <div class={styles.station__header}>
            <MediaItemDetailed
              releaseDate={null}
              mediaItemId={stationId}
              src={Utils.formatArtworkUrl(
                currentStation()?.attributes?.artwork.url,
                100,
                100,
                'webp',
                'none'
              )}
              title={currentStation()?.attributes?.name}
              artists={currentStation().attributes?.curatorName}
            />
          </div>
          <TabSwitcher
            tabs={[
              {
                name: 'Details',
                content: (
                  <div class={styles.station__details}>
                    <div class={styles.station__details__grouping}>
                      <h4 class={styles.station__details__subsubheader}>Is Live</h4>
                      <span class={styles.station__details__description}>
                        {currentStation().attributes?.isLive ? 'Yes' : 'No'}
                      </span>
                    </div>
                    <div class={styles.station__details__grouping}>
                      <h4 class={styles.station__details__subsubheader}>
                        Requires Subscription
                      </h4>
                      <span class={styles.station__details__description}>
                        {currentStation().attributes?.requiresSubscription ? 'Yes' : 'No'}
                      </span>
                    </div>
                    <div class={styles.station__details__grouping}>
                      <h4 class={styles.station__details__subsubheader}>Kind</h4>
                      <span class={styles.station__details__description}>
                        {currentStation().attributes?.kind}
                      </span>
                    </div>
                    <div class={styles.station__details__grouping}>
                      <h4 class={styles.station__details__subsubheader}>URL</h4>
                      <span class={styles.station__details__description}>
                        {currentStation().attributes?.url}
                      </span>
                    </div>
                    <div class={styles.station__details__grouping}>
                      <h4 class={styles.station__details__subsubheader}>ID</h4>
                      <span class={styles.station__details__description}>
                        {currentStation().id}
                      </span>
                    </div>
                    <div class={styles.station__details__grouping}>
                      <h4 class={styles.station__details__subsubheader}>Media Kind</h4>
                      <span class={styles.station__details__description}>
                        {currentStation().attributes?.mediaKind}
                      </span>
                    </div>
                  </div>
                ),
              ...
              },
              {
                name: 'Artwork',
                content: (
                  <div class={styles.station__artwork}>
                    <For each={artworkUrls()}>
                      {artwork =>
                        artwork?.thumbnailUrl &&
                        artwork?.url && (
                          <div class={styles.station__artwork__container}>
                            <h3 class={styles.station__artwork__header}>
                              {artwork.name}
                            </h3>
                            <A href={artwork.url} target="_blank">
                              <img src={artwork.thumbnailUrl} />
                            </A>
                            <div class={styles.station__artwork__colors}>
                              {artwork.bgColor && (
